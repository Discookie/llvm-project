...
void initializeRootValue(Value &RootValue, Environment &Env) {
  ...
  BoolValue *IsNonnull = cast_or_null<BoolValue>(RootValue.getProperty(kIsNonnull));
  ...
  // but the flow condition does not have this constraint yet. The framework provides
  // deduplication for constraints, so should not have a performance impact.
...
void NullPointerAnalysisModel::initializeFunctionParameters(
    const ControlFlowContext &CFCtx, Environment &Env) {
  const FunctionDecl &FD = cast<FunctionDecl>(CFCtx.getDecl());

  for (const auto *Param : FD.parameters()) {
    if (!Param || !Param->getType()->isAnyPointerType())
      continue;

    StorageLocation *SL = Env.getStorageLocation(*Param);
    if (!SL) {
      SL = &Env.createStorageLocation(*Param);
      Env.setStorageLocation(*Param, *SL);
    }

    auto *RootValue = Env.getValue(*Param);
    if (!RootValue) {
      RootValue = Env.createValue(Param->getType());
      if (!RootValue)
        continue;
      Env.setValue(*SL, *RootValue);
    }
...
bool NullPointerAnalysisModel::merge(QualType Type, const Value &Val1,
...
NullCheckAfterDereferenceDiagnoser::ResultType
NullCheckAfterDereferenceDiagnoser::diagnose(ASTContext &Ctx, const CFGElement *Elt,
                               const Environment &Env) {
